...
So if you try to install anything you will find that it doesn't work. So to fix this you go to /etc/yum.repos.d/ and delete all the unnecessary/unusual files, you can figure this out by comparing it to a normal /etc/yum.repos.d and looking for differences.
...
The SSH keys are linked to another vuln, so fixing the permissions by making the private key and authorized keys permissions 600 and the public key permissions 644, and both are owned by Fred gains points. The ~/.shh directory should also be 700 permissions
...
![ELK Stack Dashboard Title](red-wheelbarrow.png)
...
```
If the above command doesn't work, try this one, just get rid of docker:
```
